...
const TitanicForm = ({ isOpen, onClose, handleInputChange}) => {
  return (
        ...
        <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
          ...
          >
            <div className="inline-block align-bottom bg-white rounded-lg px-4   text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:w-1/3 sm:p-6">
              ...
              <select
                id="pclass" // Identificador del elemento
                name="pclass" // Nombre del elemento
                onChange={(e) => {
                  handleInputChange("pclass", e.target.value)
              ...
              <select
                id="sex" // Identificador del elemento
                name="sex" // Nombre del elemento
                onChange={(e) => {
              ...
              <label
                ...
                htmlFor="sibsp"
              >
                Número de hermanos o cónyuges que viajan con la persona
              </label>
              <input
                className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                id="sibsp"
              ...
              <label
                ...
                htmlFor="parch"
              >
                Número de padres o hijos que viajan con la persona
              </label>
              <input
                className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                id="parch"
              ...
              <input
                ...
                onChange={(e) => {
                    ...
                    const isInteger = /^\d+$/.test(inputValue); // Verifica si el valor es un entero
                    const formattedValue = isInteger ? parseFloat(inputValue) : parseFloat(inputValue.replace(",", ".")); // Reemplaza comas por puntos solo si no es un entero
                    const isValidNumber = !isNaN(formattedValue); // Verifica si es un número válido
                    const roundedValue = isValidNumber ? formattedValue.toFixed(2) : ""; // Redondea a dos decimales si es un número válido
                    handleInputChange("fare", roundedValue);
                  }} // Función que se ejecuta cuando se cambia el valor del input
              />
              <label
                className="block text-gray-700 text-sm font-bold mb-2 mt-2"
                htmlFor="embarked"
              >
                Puerto desde el cual la persona pasajera embarcó
              </label>
              <select
                id="embarked" // Identificador del elemento
                name="embarked" // Nombre del elemento
                onChange={(e) => {
                  handleInputChange("embarked", e.target.value)
                }} // Función que se ejecuta cuando el valor del elemento cambia
                className="mt-3 p-3 border rounded-xl bg-gray-50 w-full"
              >
                {/* opción por defecto del select */}
